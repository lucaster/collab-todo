...
<div id="todoDetail${todo.id}" class="todo">
	...
	<div class="todoTitle">
		${todo.name?.encodeAsHTML()}
		<g:link action="edit" id="${todo.id}">
			<img border=0 src="${createLinkTo(dir:'images',file:'write_obj.gif')}" alt="[EDIT]" />
		</g:link>
		<g:remoteLink action="removeTask" id="${todo.id}" update="todoDetail${todo.id}" onComplete="highlight('todoDetail${todo.id}');">
			<img border=0 src="${createLinkTo(dir:'images',file:'delete_obj.gif')}" alt="[EDIT]" />
		</g:remoteLink>
		<g:showHide update="todoDetailFull${todo.id}">
			<img border=0 src="${createLinkTo(dir:'images',file:'add_obj.gif')}" alt="[Show All]" />
		</g:showHide>
	</div>
	...
	<div id="todoDetailFull${todo.id}" class="todo" style="display: none">
		...
		<g:if test="${todo.completedDate == null}">
			Complete Task: <input type="checkbox" onclick="${remoteFunction(action:'completeTask', id:todo.id, update:'todoDetail'+todo.id, onComplete:'highlight(\'todoDetail'+todo.id+'\');' )}" /> <br />
		</g:if>
		<g:else>
...
<%--		</g:radeoxRender>--%>
		...
		<!--
			<g:remoteLink action="showNotes" id="${todo.id}" update="todoDetailNote${todo.id}">
				Notes
			</g:remoteLink>
			<br/>
